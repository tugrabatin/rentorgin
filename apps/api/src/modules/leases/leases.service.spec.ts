/**
 * Leases Service Unit Tests
 * Tests for lease contract business logic
 * 
 * Kira Servisi Birim Testleri
 * Kira sözleşmesi iş mantığı testleri
 */
...
describe('LeasesService', () => {
  ...
  describe('findExpiring', () => {
    it('should return leases expiring within specified days', async () => {
      ...
      const call = (prisma.lease.findMany as jest.Mock).mock.calls[0][0];
  ...
  describe('create', () => {
    ...
    it('should create lease with proper data transformation', async () => {
      ...
      const createCall = (prisma.lease.create as jest.Mock).mock.calls[0][0];
  ...
  describe('calculateCurrentRent', () => {
    it('should calculate escalated rent correctly', async () => {
      ...
      expect(result.currentRent).toBeGreaterThan(10000); // Should be escalated
  ...
  describe('initiateRenewal', () => {
    ...
    it('should throw NotFoundException if lease not found', async () => {
      ...
      await expect(
        service.initiateRenewal('nonexistent', 't1')
  ...
  describe('findOne', () => {
    it('should throw NotFoundException if lease not found', async () => {
      ...
      await expect(
        service.findOne('nonexistent', 't1')
      ).rejects.toThrow(NotFoundException);
    });
  ...
  describe('update', () => {
    it('should throw NotFoundException if lease not found', async () => {
      ...
      await expect(
        service.update('l1', 't1', { monthlyRent: 60000 })
      ).rejects.toThrow(NotFoundException);
    });
